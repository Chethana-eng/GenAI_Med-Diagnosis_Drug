...
def init_model():
    ...
    # Fix for torch.load with PyTorch >= 2.6
    protein_vocab = torch.load(os.path.join(root, 'protein-vocab.pt'), weights_only=False)
    smiles_vocab = torch.load(os.path.join(root, 'smiles-vocab.pt'), weights_only=False)
